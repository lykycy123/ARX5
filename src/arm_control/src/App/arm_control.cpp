...
{
    solve.get_joint(current_pos,current_vel,current_torque,rv_motor_msg,current_normal);     
}
...
void arx_arm::update_real(command cmd)
{
    ...
    solve.arm_calc1(arx5_cmd,current_pos,control_mode,target_pos,current_vel,is_teach_mode);
    if(is_starting)
    {
        init_step();
    }
        ...
        if(control_mode == 0  ||  control_mode == 1 ||control_mode==4 || control_mode==5 )
        {
                ...
                if(teach2pos_returning ){
                    ...
                    is_starting=0;
                            ...
                            gripper_pos[0] = limit<float>(gripper_pos[0] , 0, 5);
                    motor_control();
...
void arx_arm::motor_control()
{
    if(current_normal)
    {
            if (is_teach_mode) {
                ...
                CAN_Handlej.Send_moto_Cmd1(1, 0, 0, 0, 0,  solve.joint_torque[0]);usleep(200);
                CAN_Handlej.Send_moto_Cmd1(2, 0, 0, 0, 0,  solve.joint_torque[1]);usleep(200);
                CAN_Handlej.Send_moto_Cmd1(4, 0, 0, 0, 0,  solve.joint_torque[2]);usleep(200);
                CAN_Handlej.Send_moto_Cmd2(5, 0, 0, 0, 0,  solve.joint_torque[3]);usleep(200);
                CAN_Handlej.Send_moto_Cmd2(6, 0, 0, 0, 0,  solve.joint_torque[4]);usleep(200);
                CAN_Handlej.Send_moto_Cmd2(7, 0, 0, 0, 0,  solve.joint_torque[5]);usleep(200);
                CAN_Handlej.Send_moto_Cmd2(8, 0, 0, 0, 0,  0);usleep(200);
                
            } else {
                    if(play.is_playing)
                    {
                        limit_joint(target_pos);
                        CAN_Handlej.Send_moto_Cmd1(1, 150, 12, target_pos[0], 0,     solve.joint_torque[0]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd1(2, 150, 12, target_pos[1], 0,     solve.joint_torque[1]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd1(4, 150, 12, target_pos[2], 0,     solve.joint_torque[2]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd2(5, 30,  1,  target_pos[3], 0,     solve.joint_torque[3]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd2(6, 15,  1,  target_pos[4], 0,     solve.joint_torque[4]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd2(7, 10,  1,  target_pos[5], 0,     solve.joint_torque[5]   );usleep(200);
                        CAN_Handlej.Send_moto_Cmd2(8, 3 ,  1,  target_pos[6], 0,  0 );usleep(200); 
                    }
                    else
                    {
                        ...
                        CAN_Handlej.Send_moto_Cmd1(1, 150, 12, target_pos_temp[0], 0, solve.joint_torque[0]); usleep(200);
                        ...
                        if(arx5_cmd.key_o ) //开启
                        {
                            gripper_pos[0]+=0.01;
                        }
                        else if ( arx5_cmd.key_c ) //夹取
                        {
                            gripper_pos[0]-=0.01;
                        }
                        ...
                        float temp_k=8.0f; 
                        float send_cur=0;
                        send_cur=temp_k*(arx5_cmd.gripper+gripper_pos[0]-0.1-current_pos[6]);
                        send_cur=limit<float>(send_cur,-2,2);
                        
                        CAN_Handlej.Send_moto_Cmd2(8, 0, 0.1, 0, 0,send_cur );  
                        usleep(200); 
...
void arx_arm::joint_control()
{
    ...
    if(current_normal)
    {
        ...
        CAN_Handlej.Send_moto_Cmd1(1, 150, 12, ros_control_pos[0], 0, 0);usleep(200);
        CAN_Handlej.Send_moto_Cmd1(2, 210, 12, ros_control_pos[1], 0, 0);usleep(200);
        CAN_Handlej.Send_moto_Cmd1(4, 150, 12, ros_control_pos[2], 0, 0);usleep(200);
        CAN_Handlej.Send_moto_Cmd2(5, 30, 0.8, ros_control_pos[3], 0, 0);usleep(200);
        CAN_Handlej.Send_moto_Cmd2(6, 25, 0.8, ros_control_pos[4], 0, 0);usleep(200);
        CAN_Handlej.Send_moto_Cmd2(7, 10, 1  , ros_control_pos[5], 0, 0);usleep(200); 
        CAN_Handlej.Send_moto_Cmd2(8, 10, 1  , ros_control_pos[5], 0, 0);usleep(200);
